...
방법:
  ...
  2. Classic catchment: travel_time ≤ 15분인 모든 노드
  3. Thermal catchment: effective_cost = travel_time × (1 + α × penalty(UTCI)) ≤ 15분
  4. 차이 = 폭염으로 도보 접근성을 잃는 주민 범위

UTCI 산출 방식: SOLWEIG 기반 (19_solweig_utci.py 출력)
  - Open-Meteo 기상 입력 (2025-07-28~08-03, 7일 평균)
  - MRT = Tair + 0.5×√(GHI×cos_z)×SVF  (Lindberg et al. 2008)
  - 수목 캐노피 보정: utci_final = utci_solweig − CANOPY_COEFF×canopy_ratio×solar_factor
  - 참고: Bröde et al. (2012), Chen & Ng (2012)
...
def compute_catchment(G: nx.Graph, station_node: int, utci_lookup: dict,
                      utci_hourly_mean: dict, hour: int, alpha: float):
    """
    ...
    for u, v, data in G.edges(data=True):
        ...
        utci = utci_lookup.get((str(u), str(v), hour),
               utci_lookup.get((str(v), str(u), hour),
               utci_hourly_mean.get(hour, 33.0)))
...
print("UTCI 데이터 로드 중 (SOLWEIG 기반)...")
link_df = pd.read_csv(UTCI_PATH, encoding='utf-8-sig')
utci_lookup = {}
for _, row in link_df.iterrows():
    utci_lookup[(str(row['u']), str(row['v']), int(row['hour']))] = row['utci_final']
    utci_lookup[(str(row['v']), str(row['u']), int(row['hour']))] = row['utci_final']

# 시간대별 평균 UTCI — lookup 실패 시 기본값으로 사용
utci_hourly_mean = (link_df.groupby('hour')['utci_final'].mean().round(2).to_dict())
print(f"  {len(utci_lookup):,}개 로드 완료")
print(f"  시간대별 기본값 (lookup 실패 시): { {h: f'{v}°C' for h, v in utci_hourly_mean.items()} }")
...
for station_name, sinfo in STATIONS.items():
    ...
    for hour in TARGET_HOURS:
        ...
        G = G_base.copy()
        result = compute_catchment(G, sinfo['node'], utci_lookup, utci_hourly_mean, hour, ALPHA)
...
for station_name, sinfo in STATIONS.items():
    ...
    fig.suptitle(
        ...
        f"UTCI 기반 열 패널티 적용 (α={ALPHA}) | 시간예산: 15분",
